...
int GIFDecoder::DecodeGIF(char *inputGifData,int len,int* gifWidth,int* gifHeight,double *gifDelay,list<char *>& listRGBData)
{
	...
	unsigned char *data = (unsigned char *)inputGifData;//form ptr data
	size = len;
	/* begin decoding */
	do {
	...
	*gifHeight = gif.height;
	gframe = gif.frames;
	*gifDelay = (gframe->frame_delay < 10)? 0.1 :(gframe->frame_delay * 0.01);
	/* decode the frames */
	for (i = 0; i != gif.frame_count; i++) {
		...
		/*将之前获取到的RGB数据转换为ARGB数据*/
		unsigned int iXLoop, iYLoop;
		//	char *  pcARGB = (char *)malloc(gif.width*gif.height* 4);

		char *  pcARGB = new char[gif.width*gif.height* 4];
		if(pcARGB == NULL)
		{
			return NULL;
		}
		//BGR2RGB并翻转数据
		int i=0;
		for(iYLoop = 0; iYLoop < gif.height; iYLoop++)
		{
			for(iXLoop = 0; iXLoop < gif.width; iXLoop++)
			{
				pcARGB[(iYLoop*gif.width + iXLoop)*4+3] = image[((gif.height-iYLoop-1)*gif.width + iXLoop)*4+3]; ; /*A*/
				pcARGB[(iYLoop*gif.width + iXLoop)*4+2] = image[((gif.height-iYLoop-1)*gif.width + iXLoop)*4+0]; /*B*/
				pcARGB[(iYLoop*gif.width + iXLoop)*4+1] = image[((gif.height-iYLoop-1)*gif.width + iXLoop)*4+1]; /*G*/
				pcARGB[(iYLoop*gif.width + iXLoop)*4+0] = image[((gif.height-iYLoop-1)*gif.width + iXLoop)*4+2]; /*R*/
			}
		}
		listRGBData.push_back(pcARGB);
